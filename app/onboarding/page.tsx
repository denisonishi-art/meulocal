...
const steps = [
  ...
  { title: 'Conectar seus canais', text: 'Google e WhatsApp ficam conectados ao MeuLocal. O sistema operacional por trás permanece invisível.', icon: MessageCircle },
...
export default function OnboardingPage() {
  ...
  return <main className="onboardingPage">
    <header className="nav container">
      ...
      <span className="onboardingStatus">Configuração {Math.min(step + 1, 3)}/3</span>
    </header>

    <section className="onboardingHero container">
      <div className="successPill"><Check size={16}/> Seu MeuLocal está ativo</div>
      <h1>Agora vamos colocar o MeuLocal para trabalhar.</h1>
      <p>Leva poucos minutos. Você continua no nosso ambiente do começo ao fim.</p>

      <div className="onboardingCard">
        {step < 3 ? <>
          <div className="stepProgress"><i style={{width: `${((step + 1) / 3) * 100}%`}} /></div>
          <div className="stepIcon">{(() => { const Icon = steps[step].icon; return <Icon size={24}/>; })()}</div>
          <span className="stepLabel">Etapa {step + 1}</span>
          ...
          <div className="onboardingActions">
            <button className="primary" type="button" onClick={() => setStep(step + 1)}>{step === 2 ? 'Ativar operação' : 'Continuar'} <ArrowRight size={18}/></button>
            {step === 2 && <button className="linkButton" type="button" onClick={() => {setSkipped(true); setStep(3);}}>Fazer isso depois</button>}
          </div>
        </> : <div className="onboardingDone">
          <div className="stepIcon"><Check size={27}/></div>
          <h2>Configuração concluída.</h2>
          <p>{skipped ? 'Você pode enviar sua base quando quiser. O MeuLocal já está pronto para continuar a configuração.' : 'Sua operação está pronta para a próxima etapa de ativação.'}</p>
          <a className="primary" href="/">Ir para o MeuLocal <ArrowRight size={18}/></a>
        </div>}
      </div>

      <div className="privacyNote">GoHighLevel não aparece para o cliente. O MeuLocal é a única interface da experiência.</div>
    </section>
  </main>;
}
